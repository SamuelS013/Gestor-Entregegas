...
// Variable global para rastrear la factura actualmente visualizada en la modal de previa
...
formFactura.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        tempContainer.querySelector('#rf-hora').textContent = hora;
...
// Acción de Marcar Factura como Pagada + Regeneración de Imagen en Canvas
btnMarcarPagada.addEventListener('click', async () => {
    ...
    try {
        // Regenerar imagen usando Canvas para estampar la marca "PAGADA"
...
// Función auxiliar para estampar la marca roja "PAGADA" diagonalmente en la imagen
function estamparMarcaPagado(base64Src) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            const fontSize = Math.floor(canvas.width * 0.15); // Tamaño proporcional
            ctx.font = `bold ${fontSize}px Arial, sans-serif`;
            ctx.fillStyle = 'rgba(220, 53, 69, 0.4)'; // Rojo con opacidad / transparencia
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';

            // Guardar contexto y aplicar rotación en el centro
            ctx.save();
            ctx.translate(canvas.width / 2, canvas.height / 2);
            ctx.rotate(-30 * Math.PI / 180); // Inclinación diagonal hacia arriba
            ctx.fillText('PAGADA', 0, 0);

            // Borde opcional para dar efecto de sello
...
// Cargar Historial desde Firestore y Actualizar Totales
async function cargarHistorialFacturas() {
    try {
        ...
        // Actualizar métricas con la nueva lógica
